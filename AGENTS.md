...
This project is indexed by GitNexus as **code_workbench**. Use GitNexus to understand code, assess impact, and navigate safely.
...
All commands are invoked via the `Bash` tool. Add `-r code_workbench` if ambiguity arises.
...
This project uses **bd (beads)** for issue tracking. Use `bd --help` for the installed command set, or `bd ready` to see available work.
...
- Use `bd --help` if you need the current command reference
